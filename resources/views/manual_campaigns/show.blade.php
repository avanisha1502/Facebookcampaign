<style>
    ...
    .image-container .img {
        width: calc(25% - 2px);
        /* Adjust based on the gap between images */
        height: auto;
        margin-bottom: 10px;
    ...
    .image-container img {
        ...
        height: auto;
        object-fit: cover;
        /* Keeps aspect ratio and covers the area */
        border-radius: 5px;
    }
</style>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    @php
                        ...
                        $shortdescription_text = \Illuminate\Support\Str::limit($campaignAllDetails->description, 100);
                        $imageUrls = json_decode($campaignAllDetails->image, true);
                    @endphp
                    
                    <td class="hover-tooltip" data-tooltip-position="bottom"
                        data-tooltip="{{ $campaignAllDetails->headlines }}">{{ $shortheadline }}</td>
                    <td class="hover-tooltip" data-tooltip-position="bottom"
                        data-tooltip="{{ $campaignAllDetails->primary_text }}">{{ $shortprimary_text }}</td>
                    <td class="hover-tooltip" data-tooltip-position="bottom"
                        data-tooltip="{{ $campaignAllDetails->primary_text }}">{{ $shortdescription_text }}</td>
                    <td>
